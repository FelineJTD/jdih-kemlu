    ...
    <section className='defaultPadding'>
      ...
      <div className='flex flex-col lg:flex-row gap-8'>
        ...
        <div className='lg:w-3/4 bg-white rounded-lg max-h-full'>
          ...
          <div className='flex flex-col overflow-y-auto p-6 space-y-4 max-h-[24rem]'>
            { Berita.map((item, index) => {
              if (index === 0) return null
              return (
              <Link href={item.href} key={index}>
                <a className='flex p-4 space-x-4 border border-neutral-400 rounded-md hover:bg-neutral-100 duration-100 max-h-[10rem]'>
                  <div className='relative lg:min-w-0 aspect-[3/2] lg:min-h-full'>
                    <Image src={item.img} alt={item.alt} layout='fill' objectFit='cover' objectPosition='center' className='rounded-md' />
                  </div>
                  <div className='h-full overflow-hidden min-w-[67%]'>
                    <p className='text-neutral-500 mb-2 text-xs'>{item.date}</p>
                    <h4 className='font-semibold mb-2 text-sm md:text-base'>{item.title}</h4>
                    <p className='text-neutral-600 mb-2 text-sm hidden md:block'>{item.desc}</p>
                  </div>
                </a>
              </Link>
              )
